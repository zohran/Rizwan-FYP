...
export default function BillingPage() {
  ...
  const [notifyAllModal, setNotifyAllModal] = useState<{
    userId: string;
    username: string;
  } | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    const [listRes, rateRes, summaryRes] = await Promise.all([
      fetch(`/api/billing/list?${params}`),
      fetch("/api/billing/rate"),
      fetch(
        `/api/billing/summary?${new URLSearchParams({ ...(startDate && { startDate }), ...(endDate && { endDate }), ...(selectedUserId && { userId: selectedUserId }), ...(searchDebounced && { search: searchDebounced }) })}`,
      ),
    ]);
    ...
    if (listRes.ok) {
      setBillings(listData.billings ?? []);
      setTotal(listData.total ?? 0);
    }
    if (rateRes.ok) setRate(rateData.ratePerMinute ?? 1);
    if (summaryRes.ok) {
      setTotalEarnings(summaryData.totalEarnings ?? 0);
      setTotalMinutes(summaryData.totalMinutes ?? 0);
      setTotalRecords(summaryData.totalRecords ?? 0);
    } else if (listRes.ok) {
      setTotalEarnings(listData.totalEarnings ?? 0);
    }
    setLoading(false);
  ...
  useEffect(() => {
    setPage(1);
  }, [startDate, endDate, selectedUserId, searchDebounced]);
  useEffect(() => {
    fetchData();
  }, [fetchData]);
  ...
  async function updateRate() {
    ...
    const res = await fetch("/api/billing/rate", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ratePerMinute: newRate }),
    });
    if (res.ok) {
      setRate(newRate);
      setNewRate(null);
  ...
  async function sendSessionNotification(billingId: string, userId: string) {
    ...
    const res = await fetch("/api/admin/send-payment-notification", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId, billingId }),
    });
  ...
  const columns: ColumnsType<Billing> = [
    {
      ...
      render: (_: unknown, r: Billing) => (
        <Text strong>{r.userId?.username ?? "N/A"}</Text>
      ),
    },
    { title: "Minutes", dataIndex: "totalMinutes", key: "minutes" },
    {
      title: "Rate/min",
      key: "rate",
      render: (_: unknown, r: Billing) => `$${r.ratePerMinute.toFixed(2)}`,
    },
    {
      title: "Amount",
      key: "amount",
      render: (_: unknown, r: Billing) => (
        <Text strong style={{ color: "#38a169" }}>
    ...
    <div className="animate-fade-in">
      <Row justify="space-between" align="middle">
        <Col>
          ...
          <Text type="secondary">
            Manage rates, filter records, send payment notifications
          </Text>
        </Col>
        <Col>
          <Select
      ...
      <Row gutter={[24, 24]} style={{ marginTop: 24, marginBottom: 24 }}>
        ...
        <Col xs={24} sm={8}>
          <Card>
            <Statistic title="Total Records" value={totalRecords} />
          </Card>
        </Col>
      </Row>
      ...
      <Card title="Rate per Minute" style={{ marginBottom: 24 }}>
        <Space align="end">
          <div>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: "#4fd1c5" }}>
              ${rate.toFixed(2)} / min
            </div>
          </div>
          <div>
      ...
      <Modal
        ...
        onOk={() =>
          notifyAllModal && sendAllNotification(notifyAllModal.userId)
        }
        ...
        {notifyAllModal && (
          <div>
            <p>
              ...
              <strong>{notifyAllModal.username}</strong> for their{" "}
              <strong>total balance across all sessions</strong>?
            </p>
            <p style={{ marginTop: 8, color: "#999" }}>
              The client will receive a real-time notification with the total
              amount due.
            </p>
          </div>
        )}
