...
export default function ClientDashboardPage() {
  ...
  useEffect(() => {
    if (!session) return;
    const i = setInterval(fetchSession, 30000);
    ...
    socket.on(
      ...
      (payload: { message: string; amount: number }) => {
        notification.info({
          ...
          icon: <DollarOutlined style={{ color: "#4fd1c5" }} />,
          placement: "topRight",
          duration: 8,
        });
      },
  ...
  useEffect(() => {
    ...
    const i = setInterval(() => {
      setSession((p) => {
        if (!p || p.remainingTime <= 0) return p;
        if (p.remainingTime <= 300 && !showWarning) setShowWarning(true);
    ...
    try {
      ...
      const stored =
        typeof window !== "undefined" ?
          sessionStorage.getItem("clientLoginImage")
        : null;
      if (stored) {
        imageBase64 = stored;
      } else {
        try {
          const b64 = await captureWebcam();
          const sr = await fetch("/api/auth/save-login-image", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ imageBase64: b64 }),
          });
          const sj = await sr.json();
          if (sr.ok && sj.imageBase64) {
            imageBase64 = sj.imageBase64;
            sessionStorage.setItem("clientLoginImage", sj.imageBase64);
          } else throw new Error("Failed");
        } catch {
          setWebcamError("Camera access denied. Session blocked.");
          setIsStarting(false);
          return;
        }
      }
      const res = await fetch("/api/sessions/start", {
        ...
        body: JSON.stringify({
          duration,
          machineId: `client-${Date.now()}`,
          imageBase64,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        setWebcamError(data.error ?? "Failed to start session");
        setIsStarting(false);
        return;
      }
      setSession(data.session);
            ...
            {DURATIONS.map((d) => (
              <Col span={8} key={d}>
                <Button
                  ...
                  style={{ height: 80, fontSize: 18, fontWeight: 600 }}
                >
                  {d} min
                </Button>
              </Col>
  ...
  return (
    ...
    >
      ...
      {showWarning && (
        <Alert
          message="5 minutes remaining — session ends automatically"
          type="warning"
          showIcon
      ...
      <Button
        ...
        style={{ marginTop: 32, width: 280, height: 48 }}
      >
        End Session
      </Button>
      ...
      <Modal
        ...
        footer={
          <Button type="primary" onClick={() => setShowWarning(false)}>
            OK
          </Button>
        }
      >
